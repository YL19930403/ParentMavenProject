...
public interface IUserDao {
    ...
    @Results({
        ...
        @Result(column = "phone_no", jdbcType = JdbcType.BIGINT, property = "phone_no", javaType = BigInteger.class),
        @Result(column = "status", jdbcType = JdbcType.TINYINT, property = "status", javaType = Integer.class),
        // 根据用户id去user_role表拿数据
        @Result(column = "id", property = "roleInfos", javaType = java.util.List.class, many = @Many(select = "cn.itcast.dao.IUserRoleDao.findByUserId")),
    })
    public UserInfo findUserInfoById(Integer id) throws Exception;

    @Select("SELECT * FROM `role` WHERE  NOT IN (SELECT role_id FROM `user_role` WHERE user_id=#{id})")
    @Results({
